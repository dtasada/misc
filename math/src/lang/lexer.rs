...
impl Lexer {
        ...
        let (single_token, single_token_length) = match char {
            ...
            _ => {
                let word = self.input[self.position..]
                    ...
                    .collect::<Vec<&str>>()[0]
                    .to_string();
                (Token::Unknown(word.clone()), word.len())
            }
        };

        match single_token {
            Token::Number(_) | Token::Ident(_) => return (single_token, single_token_length),
            _ => {}
        };

        match self.peek(starting_offset) {
            Some(next_char) => {
                let (next_token, next_token_length) =
                    self.get_token(next_char, starting_offset + 1);

                if next_token_length > 1 {
                    return (single_token, single_token_length);
                } else {
                    return match single_token {
                        ...
                        Token::Number(_) | Token::Ident(_) | Token::Whitespace(_) => {
            ...
            None => (single_token, single_token_length),
        }
    }
    ...
    fn read_number(&mut self) -> (Token, usize) {
        ...
        if let Some(mat) = re.find(word) {
            ...
            (Token::Unknown(word.to_string()), word.len())
        }
    }

    fn read_ident(&mut self) -> (Token, usize) {
        let not_ident = Regex::new(r"[^a-zA-Z0-9_]").unwrap();
        let word = self
            .input
        ...
        (Token::Ident(ident.to_string()), ident.len())
    }
